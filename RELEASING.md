...
11. **Schema 1 compliance** – implement the full Schema 1 register map per the device appendix in [NW-Device-Specification](https://github.com/NorthernWidget/NW-Device-Specification):
    ...
    - **Page 2 (0x20–0x3F, SRAM):** the universal Block 0 – status `0x20` (ready, per-chip fault bits, pan-fault), control `0x21` (trigger, chip select, sleep), reading counter `0x22–0x23`, readings requested `0x44–0x45` (writable count for batches), device config `0x46`, report `0x47` (the Report register: the latched code of the device's most recent report, fault or notice) – then device data from `0x48` per the appendix (Page 3 continues data past 24 bytes)
...
3. **Register map** – Page 0 served from the top of EEPROM (CRC checked, patch substituted); Page 2 universal Block 0 with the rules in the spec (only `0x41`, `0x44–0x45`, and `0x46` writable; atomic rewrite; ready cleared at reading start, set with the counter increment; latched report cleared by a control write); data from `0x48`; Page 2 for calibration.
