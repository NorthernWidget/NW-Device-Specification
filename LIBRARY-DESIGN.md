...
- Burst on a dead chip: the device does not retry a failed power-up on every trigger of the burst, and the library stops the burst at a power-up fault (chip fault kind 1 or 5), so an arbitrarily large N costs nothing when the chip is absent.
- Burst abandonment (device-side timer, Apis 2 s): DEFERRED (Andy 2026-09-22) until "burst" itself is thought through; see #25 item 5.
