...
- Reports and faults: `faulted(chip)`, `anyFault()`, `reportChip()`, `reportKind()`, `printReport(Print&)`, `reportNote()`; `printStatus(Print&)`, one status line (name, serial, versions, the last report, Pages 0–2 in hex) for the logger's status file. **Status file (Andy 2026-09-23):** the logger keeps a separate status log beside the data file; whenever a device's `reportKind()` is not zero after a reading, and at boot, it writes its timestamp, then the device's `printStatus()` line. Faults, resets, stored calibrations and any device-specific notice all arrive through that one channel; the logger side is Margay_Library #7 (queue B4) and waits on the SD-writing review (queue 9). **Columns (2026-09-23):** the row is `Time,Trigger,Device,Serial,HW,FW,FWCommit,Lib,LibCommit,Code,Note,Page0,Page1,Page2`: the device's hardware version and firmware patch from Page 0, its build commit from Block 3 (eight hex digits, `+` for a dirty tree, blank when the bytes are zero), the reading library's `<LIB>_LIBRARY_VERSION` and `<LIB>_LIBRARY_COMMIT`, the Report code and its note token, then Pages 0–2 as space-delimited 8-byte blocks in hex. Nothing in the row is decoded beyond the note token; Project-Codering decodes the pages after the fact.
...
Verified by the reviewer before review: Apis harness passes (844 transactions); Apis_Demo baseline sizes uno 13594 B flash / 1008 B RAM, NW1284p 12980 / 998.

1. **Ordering conflict to resolve (Andy's call).** §0 and §10 say Core is extracted after Walrus and Haar reproduce; §11 extracts from Apis first. Resolved (Andy 2026-09-23): extract now; Core stays **unversioned** (library.properties `version=0.0.0` placeholder, no tags) and unregistered until the whole overhaul is finished; nothing is registered until then. Core's first version and its Library Manager registration precede the first consumer release (decision 4). Interface additions from Andy's question (2026-09-23): `readBytes` chunks at the 32-byte Wire buffer (Page 3 devices); `begin()` retries the acknowledge for a device-declared boot time (the logger calls it right after switching the rail on); the wait step must not require a prior request (free-running devices); accessors for the config byte and the sleep bit; the Wire stub models the 32-byte limit.
2. **Split the handshake.** `takeReading` becomes `requestReading(mask)` + `waitReading()` (counter poll with the ceiling) + `captureReading()` (status/fault). Haar's non-blocking `updateMeasurements(false)` triggers and returns, then waits later (Haar.cpp:69–103); the monolith cannot serve it. `batchFaulted` moves into Core, keyed on the selected mask, since the rule is universal (README Block 0 rules).
3. **Walrus-on-Core is gated on Walrus firmware,** not on Core: the firmware serves only the ready bit, no counter/trigger/fault, and one or two bytes per request (Walrus_I2C_5BA.ino:241–255, 444–455), so Core's 16-byte Page 0 read cannot talk to it. Project-Walrus #15–#18 plus Block 0 first.
...
## 12. Logger core: what Margay and Okapi share *(surveyed 2026-09-23; Andy: "Logger Core sounds great"; BUILT the same day: NW_Logger 88b5c28..9314cc2, Margay 9631bf0..37e3d54, Okapi 048b5fa)*
...
| SD self-test (the Hamlet write and read-back) | 460–491 | 333–362 | Margay (Okapi's PASS/FAIL at 366–367 tests the wrong flag: a read-back mismatch prints PASS) |
...
**As built (2026-09-23).** `NW_Logger` holds everything in the verbatim table plus `begin()` in pieces (`acceptAddresses`, `readIdentity`, `serialTimeSet`, `attachLoggerInterrupts(buttonOnPCINT)`, `ledReport`, `attachExtInt`), `clockUnix()`, and the hooks `begin(vals, n, header)`, `addDataPoint(update)`, `sleepNow()`, `dataHeader()` and `afterLogEvent()` (Okapi's backhaul). The rails and the bus switch did not need hooks yet: each logger kept its own `sleepNow`/`turnOffSDcard`/`turnOnSDcard`, and Okapi's `i2cState` is its own name for the same pin. Okapi took Margay's card layout, the status file, Page 0 and camelCase names with deprecated PascalCase forwarders, and fixed the five bugs the survey found on the way. Cost: Apis_Margay 63250 to 65144 B flash, 3084 to 3136 B RAM. (Correction 2026-09-24: the NorthernWidget core inherits arduino:avr's flags and already links with `-flto`; the growth is what virtual dispatch costs. `run()`, `begin()` and the file writers now reach `addDataPoint()`, `sleepNow()` and `dataHeader()` through the vtable, so the compiler no longer specialises them for the one global logger object and no longer inlines `sleepNow`, `logStr` and `statusRow` into their single callers: `begin()` +654 B, `addDataPoint()` +246 B, `sleepNow()` +372 B, `logStr()` +274 B, `statusRow()` +274 B, measured by symbol. Marking `Margay` `final` recovers nothing, since the calls are made inside the base.) Okapi grew by what it gained: Apis_Okapi 57946 to 63146 B, 2393 to 3099 B.

**Recommendation (as made).** Build `NW_Logger` beside NW_Core as a base class that Margay and Okapi inherit, in the order (1) the verbatim block above with Margay's versions where the two differ, verified by the Margay harness and the eighteen NW-Tests sketches staying byte-identical and compiling, (2) the six hooks, (3) Okapi's Page 0 read and `NW_Pages`, which brings it the card layout and the status file for free. Okapi has no deployed units (it is a prototype), and its card layout and file names can therefore follow Margay's without a migration. The blast radius: Okapi_Library's every method name, its README and demo, and the nine Okapi sketches in NW-Tests; Margay's public surface does not change.
