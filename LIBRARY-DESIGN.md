...
1. **Ordering conflict to resolve (Andy's call).** §0 and §10 say Core is extracted after Walrus and Haar reproduce; §11 extracts from Apis first. Proposed resolution: extract now from Apis, but Core stays 0.x and unregistered until Walrus and Haar consume it; Core 1.0 and its Library Manager registration precede the first consumer release (decision 4). Apis's release already waits on the bench, so this costs no time.
